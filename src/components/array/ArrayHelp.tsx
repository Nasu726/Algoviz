...
const OpsHelp: React.FC<{ maxOps: number; kind: 'stack' | 'queue' | 'deque' }> =
    ...
    <>
        ...
        <ul>
            {kind === 'stack'
                ? <li>箱を縦に積んだ入れ物。<b>口は上</b>で、下から積み上がる</li>
                : <li>箱を横に並べた入れ物。値は左から詰めて置く</li>}
            <li><b>入れ物の外に出た値</b>は枠が無い。入る前と、出たあとの一瞬だけ見える</li>
            <li>橙色が<b>次に出る値</b></li>
            <li>緑色が今出入りした値</li>
        </ul>
        ...
        <p>
            ...
            頭から流します。取り出した値は<b>次の手で消えます</b>。
        </p>

        <h3>3. 出し入れする端</h3>
        {kind === 'stack' && (
            <ul>
                <li><b>入れるのも出すのも上</b> (最後に入れたものが最初に出る)</li>
                <li>この出方を LIFO という</li>
                <li>下に埋まっている値は、上が空くまで出てこない</li>
            </ul>
        )}
        {kind === 'queue' && (
            <ul>
        ...
        <ul>
            <li>操作の数の上限は {maxOps}。箱の数は、同時に入る最大の数になります</li>
        </ul>
