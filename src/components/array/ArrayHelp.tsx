...
const RadixHelp: React.FC<{ maxValues: number }> = ({ maxValues }) => (
    <>
        <Screen>
            ...
            <li>赤色が今見ている値 (数えているとき)。緑色が動かした値</li>
            <li>灰色は位置が確定した値 (最後のビットを戻すときだけ)</li>
        </Screen>

        <h3>2. ビットごとに4回まわす</h3>
        <ul>
            <li><b>数える</b>：元の配列を左から見て、今のビットが 0 の値を数える</li>
            <li><b>配置する</b>：元の配列を左から見て、0 なら別配列の前から、
                1 なら「0 の個数」の位置から後ろへ、それぞれ詰めて置く</li>
            <li><b>戻す</b>：別配列を左から元の配列へ戻す</li>
...
const OpsHelp: React.FC<{ maxOps: number; kind: 'stack' | 'queue' | 'deque' }> =
    ...
    <>
        ...
        <p>
            ...
            頭から流します。取り出したマスは<b>外へ出たあと消えます</b>。
        </p>
