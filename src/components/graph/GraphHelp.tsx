...
const AlgorithmSection: React.FC<{ variant: GraphVariant }> = ({ variant }) => {
    ...
    if (variant === 'kruskal') return (
        <>
            ...
            <ul>
                <li>1本の辺は2ステップ。まず橙で「見る」、次に採用 (緑) か却下 (灰) を「決める」。
...
const AutomatonHelp: React.FC<{ maxNodes: number }> = ({ maxNodes }) => (
    <>
        ...
        <h3>2. 遷移規則の作り方</h3>
        <ul>
            <li><b>ランダム生成</b>：状態数とアルファベットを決めると、決定性の遷移規則を作る。
                初期状態からすべての状態へ届くようにしたうえで、<b>一部の遷移はわざと作らない</b></li>
        ...
        <ul>
            <li>1ステップは「記号を1つ読んで遷移する」。入力を読み切った時点も1ステップとして、
                そこで受理か拒否かが決まる</li>
            <li>読んだ記号の遷移が定義されていないと、その時点で拒否になる</li>
